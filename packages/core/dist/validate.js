...
const CAMERAS = new Set(['push_in', 'pull_out', 'pan_left', 'pan_right', 'float', 'orbit', 'static']);
const TRANSITIONS = new Set(['cut', 'fade', 'slide', 'zoom']);
const STYLES = new Set(['fast-commerce', 'clean-product', 'social-proof']);
function cleanText(value, fallback, max) {
    const text = String(value ?? '').replace(/\s+/g, ' ').trim().slice(0, max);
    return text || fallback;
}
function normalizeDurations(scenes, target) {
    if (!scenes.length)
        return scenes;
...
export function sanitizeDirector(raw, expectedProductName, expectedDuration) {
    if (!raw || typeof raw !== 'object')
        throw new Error('invalid_director_object');
    const input = raw;
    const sourceScenes = Array.isArray(input.scenes) ? input.scenes : [];
    if (!sourceScenes.length)
        throw new Error('director_has_no_scenes');
    ...
    const scenes = sourceScenes.slice(0, maxScenes).map((scene, index) => {
        const template = TEMPLATES.has(scene?.template) ? scene.template : (index === 0 ? 'problem_hook' : 'lifestyle');
        const camera = CAMERAS.has(scene?.camera) ? scene.camera : 'push_in';
        const transition = TRANSITIONS.has(scene?.transition) ? scene.transition : 'cut';
        return {
